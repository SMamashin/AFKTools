...
script_properties('work-in-pause') -- õóé
...
function handleTaxDialog(dialogId, style, title, text)
...
Àâòîðû: Bakhusse & Mamashin.]]
	...
	sampRegisterChatCommand('autorul', openchestrullet)
	...
	if afksets.v then
		...
		if menunum == 0 then
			local buttons = {
				...
				{fa.ICON_COGS .. u8(' Êàñòîìèçàöèÿ'),6,u8('Âûáîð ñòèëÿ, èçìåíåíèå òåìû ñêèðïòà')},
		...
		elseif menunum == 5 then
			...
			elseif imgui.CollapsingHeader(u8'v3.0 (Ãëîáàëüíîå îáíîâëåíèå, TG Notifications, êàñòîìèçàöèÿ è äð.)') then
				...
				imgui.Separator()
			elseif imgui.CollapsingHeader(u8'v3.1, v3.2 (Èñïðàâëåíèÿ îøèáîê, äîëãàÿ ïàóçà, íîâûå ôóíêöèè)') then
    ...
    if id == 220 then
        ...
        if msgType == 17 then
            ...
            if len > 0 and len < 4096 then
				...
				if text:find('event.arizonahud.playerSatiety') then
...
function sampev.onShowDialog(dialogId, dialogStyle, dialogTitle, okButtonText, cancelButtonText, dialogText)
    ...
    if handleTaxDialog(dialogId, style, title, text) then
        return false
    end
