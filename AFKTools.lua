...
changelog9 = [[
	...
	v3.5.1
		...
		· Исправлена работа AFKTools reloader, теперь он перезагружает AFKTools когда он выключен
...
function sendOnlinePing()
    ...
    local safe_id = tostring(name)

    local encoded_id = u8:encode(safe_id):gsub("([^%w])", function(c)
        return string.format("%%%02X", string.byte(c))
    end)

    local encoded_srv = u8:encode(tostring(srv)):gsub("([^%w])", function(c)
        return string.format("%%%02X", string.byte(c))
    end)

    local full_url = "https://afk.online.bakh.us/ping/?id=" .. encoded_id .. "&server=" .. encoded_srv

    async_http_request2(full_url, '', function(result) end)
end
