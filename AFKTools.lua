script_name('AFK Tools') -- я ебал в рот разрабов арз
...
-- Переменная для хранения правил чата с сервера
local remoteRulesText = u8"Загрузка правил..."
...
changelog8 = [[
	...
	v3.4.1 HOTFIX
		...
		· В связи с блокировкой Telegam на территории РФ, добавлена возможность подключить прокси для корректной работы уведомлений
		  Примечание: В случае если у вас нет прокси, вы можете использовать встроенные прокси скрипта
		· Была увеличена задержка перед закрытием инвентаря во время работы автооткрытия, в случае если у вас много тайников
...
changelog9 = [[
	...
	v3.5.1
		...
		· Добавлена кнопка скрытия ника на главной странице скрипта
		· Исправлены микро-фризы
		· Исправлена работа AFKTools reloader, теперь он перезагружает AFKTools по команде корректно
		· Исправлен краш при установке кастомного инфобара
		· В основных настройках можно включить автоматический перезапуск скрипта в случае краша, для работы требуется AFKTools reloader

	v3.5.2

		· Исправлен краш скрипта из-за уведомлений в TG
		· Исправлены фризы

	v3.5.3
	
		· Исправлено уведомление о PayDay на новое, в связи с обновлением
		· Кнопка на скачивания скрипта Автооткрытие /boxset была заменена на NewMoneySeparator
		· Добавлен чекбокс для разделения денег в уведомлениях
...
В случае неработоспособности TG Notifications - используйте прокси!
...
function updatePlayerLevel()
    ...
    if not ok or not myId or myId < 0 then return end

    local success, score = pcall(function()
        return sampGetPlayerScore(myId)
    end)

    if success and score ~= nil then
        cachedPlayerLevel = tostring(score)
    end
end
...
function buildArizonaMoney(m, kk, k)
    local total = 0
    ...
    if k then
        total = total + parseKValue(k)
    end

    return formatNumberDots(total)
end

function convertMoneyTags(text)
    if not mainIni.config.separate then
        return text
    end
    ...
    text = text:gsub(":M:%s*(%d+)%s*:KK:%s*(%d+)%s*:K:%s*([%d%.]+)", function(m, kk, k)
        return buildArizonaMoney(m, kk, k)
    end)

    text = text:gsub(":M:%s*(%d+)%s*:KK:%s*(%d+)", function(m, kk)
        return buildArizonaMoney(m, kk, nil)
    end)

    text = text:gsub(":M:%s*(%d+)%s*:K:%s*([%d%.]+)", function(m, k)
        return buildArizonaMoney(m, nil, k)
    end)

    text = text:gsub(":KK:%s*(%d+)%s*:K:%s*([%d%.]+)", function(kk, k)
        return buildArizonaMoney(nil, kk, k)
    end)

    text = text:gsub(":M:%s*(%d+)", function(m)
        return buildArizonaMoney(m, nil, nil)
    end)

    text = text:gsub(":KK:%s*(%d+)", function(kk)
        return buildArizonaMoney(nil, kk, nil)
    end)

    text = text:gsub(":K:%s*([%d%.]+)", function(k)
        return buildArizonaMoney(nil, nil, k)
    end)
...
function parseInfobarTemplate(template)
    ...
    local lvl = cachedPlayerLevel or "?"
...
function getTgBaseUrl()
    local base = 'https://api.telegram.org/bot' -- По умолчанию (0)
    
    if tgnotf.proxy_type.v == 1 then
        base = 'https://tg.bakh.us/bot'
    elseif tgnotf.proxy_type.v == 2 then
        -- Берем кастомный URL, чистим от лишних слэшей и добавляем /bot
...
function processing_telegram_messages(result)
    if result and result ~= "" and tgnotf.state.v then
        local status, proc_table = pcall(decodeJson, result)
        if status and type(proc_table) == "table" and proc_table.ok then 
            if proc_table.result and #proc_table.result > 0 then
                local res_table = proc_table.result[1]
                if res_table and res_table.update_id ~= updateid and res_table.message then
                    updateid = res_table.update_id
                    if res_table.message.text and res_table.message.from then
                        local message_from_user = res_table.message.text
                        local from_id = res_table.message.from.id
                        if from_id == tonumber(tgnotf.user_id.v) then 
                            ...
                            elseif text:match('^!send ') then
                                local cmd_text = text:sub(1, text:len() - 1):gsub('!send ','')
                                sampProcessChatInput(cmd_text)
                                sendtgnotf('Сообщение "' .. cmd_text .. '" было успешно отправлено в игру')
                            elseif text:match('^!sendcode ') then
                                local cmd_text = text:sub(1, text:len() - 1):gsub('!sendcode ','')
                                sampSendDialogResponse(8928, 1, false, cmd_text)
                                sendtgnotf('Код "' .. cmd_text .. '" был успешно отправлен в диалог')
                            elseif text:match('^!sendvk ') then
                                local cmd_text = text:sub(1, text:len() - 1):gsub('!sendvk ','')
                                sampSendDialogResponse(7782, 1, false, cmd_text)
                                sendtgnotf('Код "' .. cmd_text .. '" был успешно отправлен в диалог')
                            elseif text:match('^!gauth ') then
                                local cmd_text = text:sub(1, text:len() - 1):gsub('!gauth ','')
                                sampSendDialogResponse(8929, 1, false, cmd_text)
                                sendtgnotf('Код "' .. cmd_text .. '" был успешно отправлен в диалог')
...
function loadRules()
	lua_thread.create(function()
		while true do
			local ok, response = pcall(requests.get, 'https://chat.bakh.us/adminMessage')
			if ok then
				...
				if response.status_code == 200 then
					...
					if data then
						if data.success and data.text then
							remoteRulesText = data.text
						else
							remoteRulesText = u8"Не удалось загрузить правила (нет данных)"
						end
					else
						remoteRulesText = u8"Ошибка парсинга JSON"
					end
				else
					remoteRulesText = u8"Ошибка сервера (код: " .. response.status_code .. ")"
				end
			else
				remoteRulesText = u8"Ошибка подключения к серверу"
			end
			wait(600000) -- 10 min
		end
	end)
...
Авторы: Bakhusse & Mamashin.]]
    ...
    while true do 
        ...
        if os.time() - last_online_update >= 120 then
		    updateOnlineCount()
		    last_online_update = os.time()
		end
		if idsshow then
...
function imgui.OnDrawFrame()
	...
	if afksets.v then
		...
		local NicknameDisplay = isNickVisible and "********" or acc
		local icon = isNickVisible and (fa.ICON_EYE) or (fa.ICON_EYE_SLASH)
		...
		imgui.SetCursorPos(imgui.ImVec2(90,35))
		imgui.Text(u8('Аккаунт: ' ..NicknameDisplay)) imgui.SameLine() if imgui.Button((icon .. "##hide_nick"), imgui.ImVec2(25, 20)) then isNickVisible = not isNickVisible end
		...
		if menunum == 0 then
			...
			local total_width = 40 + 150 + 150 + 40 + (3 * 8)
			local window_width = imgui.GetWindowWidth()
			local center_offset = (window_width - total_width) / 2
			...
			imgui.SameLine()
			if imgui.Button(fa.ICON_COMMENT, imgui.ImVec2(40, 30)) then chat_window.v = not chat_window.v end
			imgui.EndGroup()
		...
		elseif menunum == 1 then
			...
			imgui.BeginGroup()
			imgui.Checkbox(u8('Автоматическая перезагрузка'),autoreload)
			imgui.SameLine()
			imgui.TextQuestion(u8('Автоматическая перезагрузка скрипта в случае краша скрипта, для работы требуется AFKTools reloader v2.1! Скачать его можно в разделе "Скрипты по отдельности".'))
			imgui.Checkbox(u8('Отключить разделение средств'),separate)
			imgui.SameLine()
			imgui.TextQuestion(u8('При получении уведомлений скрипт автоматически будет нормализовать деньги из М, КК, К в обычное число. Например, из М1 КК88 К379.854 в 1.088.379.854. Работает только на уведомления! Для визуального восприятия установите отдельный NewMoneySeparator в разделе "Скрипты по отдельности".'))
			imgui.EndGroup()
			...
			-- AFKTools reloader --
			imgui.SameLine()
			if imgui.Button(u8('AFKTools reloader v2.1')) then
				...
				sampAddChatMessage("{FF8000}[AFKTools]{FFFFFF} AFKTools reloader v2.1 успешно загружен! Нажмите Ctrl+R для перезапуска MoonLoader", -1)
            end
            imgui.SameLine()
			imgui.TextQuestion(u8('Данный скрипт сможет перезапустить AFKTools в случае его краша по команде !afkreload из VK/TG. Автоматически перезагружает скрипт если установлен чекбокс в основных настройках.'))
			-- NewMoneySeparator --
			imgui.SameLine()
			if imgui.Button(u8('NewMoneySeparator')) then
				downloadUrlToFile('https://github.com/SMamashin/AFKTools/raw/main/scripts/NewMoneySeparator.lua',
                   'moonloader\\NewMoneySeparator.lua', 
                   'NewMoneySeparator.lua')
				sampAddChatMessage("{FF8000}[AFKTools]{FFFFFF} NewMoneySeparator успешно загружен! Для работы скрипта требуется перезайти в игру!", -1)
            end
            imgui.SameLine()
			imgui.TextQuestion(u8('Скрипт фиксит новый формат денег на Arizona RP. Вместо: [M 18 KK 852 K 564.994] Ты будешь видеть: [18.852.564.994]'))
			-- Libs --
		--[[	imgui.SameLine()
		...
		elseif menunum == 9 then
			...
			if tgnotf.state.v then
				...
				imgui.SameLine()
				imgui.EndGroup()
				imgui.SameLine(700) -- Подбери число под ширину своего окна, чтобы уехало вправо
				imgui.BeginGroup()
				    imgui.Text(u8'Настройки прокси:')
				    
				    if imgui.RadioButton(u8'Без прокси', tgnotf.proxy_type.v == 0) then 
				        tgnotf.proxy_type.v = 0 
				    end
				    imgui.SameLine()
				    imgui.TextQuestion(u8'Использоваться будет api.telegram.org, если у вас нет сложностей в работе уведомлений.')

				    if imgui.RadioButton(u8'Встроенные прокси', tgnotf.proxy_type.v == 1) then 
				        tgnotf.proxy_type.v = 1 
				    end
				    imgui.SameLine()
				    imgui.TextQuestion(u8'Использоваться будет tg.bakh.us. Это прокси от разработчиков AFKTools, который является альтернативой api.telegram.org. Рекомендуется если доступ к Telegram API ограничен.')

				    if imgui.RadioButton(u8'Собственные прокси', tgnotf.proxy_type.v == 2) then 
				        tgnotf.proxy_type.v = 2 
				    end
				    imgui.SameLine()
				    imgui.TextQuestion(u8'Вы можете использовать собственные прокси для работы Telegram API. Примечание: Нужен VPS/VDS сервер для развертывания собственных прокси для Telegram API.')

				    -- Если выбраны свои прокси, показываем поле для ввода ссылки
				    if tgnotf.proxy_type.v == 2 then
				        imgui.PushItemWidth(150)
				        imgui.Text(u8'URL прокси')
...
function sampev.onServerMessage(color,text)
	...
	if vknotf.ispayday.v then
	    ...
	    if vknotf.ispaydaystate then
	        ...
	        if text:find('==========================================================================') and vknotf.ispaydaytext ~= '' then
	...
	if tgnotf.ispayday.v then
	    ...
	    if tgnotf.ispaydaystate then
	        ...
	        if text:find('==========================================================================') and tgnotf.ispaydaytext ~= '' then
...
function threadHandleGet(runner, url, resolve, reject)
	local t = runner(url)
	local r = t:get(0)
...
function async_http_get(url, resolve, reject)
	local runner = requestRunnerGet()
	if not reject then reject = function() end end

	lua_thread.create(function()
		threadHandleGet(runner, url, resolve, reject)
	end)
end
...
function sendOnlinePing()
    ...
    local result, my_id_int = sampGetPlayerIdByCharHandle(PLAYER_PED)
    if not result then return end

    local name = sampGetPlayerNickname(my_id_int)
    local srv = sampGetCurrentServerName()
    local ver = thisScript().version

    local url = 'https://afk.online.bakh.us/ping?id=' .. urlencode(name) .. '&server=' .. urlencode(tostring(srv)) .. '&version=' .. urlencode(ver)
...
function updateOnlineCount()
    local url = "https://afk.online.bakh.us/online"
    
    async_http_get(url, function(result)
        if result and result ~= "" then
            local count = result:match('"count"%s*:%s*(%d+)')
            if count then
...
function processOnlineCountTask()
    if not online_task then return end

    local status = online_task:status()
    if status == "completed" then
        local ok, result = pcall(function()
            return online_task:get()
        end)
...
function draw_chat_window()
    ...
    if imgui.Begin(u8"AFK Tools | Межсерверный чат! [beta]", chat_window, imgui.WindowFlags.NoCollapse) then
        ...
        if not is_chat_authorized then
			...
			imgui.TextWrapped(remoteRulesText)
...
function saveini()
	...
	mainIni.config.autoreload = autoreload.v
	mainIni.config.separate = separate.v
