script_name('AFK Tools') -- фиксики пришли фиксить твою жопу 2
...
function get_telegram_updates()
    if tgnotf.state.v then 
        ...
        local args2 = ''
        while true do
            -- Формируем актуальный URL на каждой итерации
...
function async_http_get(url, resolve, reject)
    ...
    lua_thread.create(function(u)
        local req = require 'requests'

        local ok, response = pcall(req.get, u, { timeout = 5 })

        if ok and response and response.status_code == 200 then
            resolve(response.text)
        else
            reject(response and response.status_code or "request_failed")
        end
    end, url)
end

local function url_encode_ping(str)
...
function sendOnlinePing()
    ...
    local encoded_id = url_encode_ping(name)
    local encoded_srv = url_encode_ping(srv)

    local url = string.format(
        "https://afk.online.bakh.us/ping?id=%s&server=%s",
        encoded_id,
        encoded_srv
    )
    ...
    async_http_get(url,
        function(result)
            sampAddChatMessage("{00FF00}PING OK: " .. tostring(result), -1)
        end,
        function(err)
            sampAddChatMessage("{FF5555}PING ERROR: " .. tostring(err), -1)
        end
    )
end
...
function updateOnlineCount()
    lua_thread.create(function()
        local ok, response = pcall(function()
            return requests.get("https://afk.online.bakh.us/online", {
                timeout = 5,
                verify = false
            })
        end)
        ...
        local status = response.status_code or response.status
        if status ~= 200 then
            return
        end

        local body = response.text or response.body or ""
        local count = body:match('"count"%s*:%s*(%d+)')
